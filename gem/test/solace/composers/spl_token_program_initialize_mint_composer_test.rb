...
describe Solace::Composers::SystemProgramCreateAccountComposer do
  ...
  describe 'initialize mint' do
    ...
    # In this spec, we must create the account first before initializing the mint. This is
    # because the initialize mint instruction requires an existing account. To do this,
    # we will use the SystemProgramCreateAccountComposer as the first instruction in the
    # transaction, followed by the SplTokenProgramInitializeMintComposer.
    ...
    let(:initialize_mint_composer) do
      Solace::Composers::SplTokenProgramInitializeMintComposer.new(
        decimals: 6,
    ...
    it 'creates the mint account with correct balance and owner' do
      ...
      assert_equal @mint_account_after['owner'], Solace::Constants::TOKEN_PROGRAM_ID
    end
  end
