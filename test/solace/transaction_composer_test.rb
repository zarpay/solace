...
describe Solace::TransactionComposer do
  ...
  let(:composer) { Solace::TransactionComposer.new(connection:) }
  ...
  describe '#add_instruction' do
    it 'adds instruction composer and returns self for chaining' do
      result = composer.add_instruction(transfer_composer_1)

      assert_equal composer, result
      assert_equal 1, composer.instruction_composers.length
      assert_equal transfer_composer_1, composer.instruction_composers.first
    end
    ...
    it 'handles multiple instruction composers with account deduplication' do
      composer
        .add_instruction(transfer_composer_1)
        .add_instruction(transfer_composer_2)
      ...
      # Random should be writable (from transfer_composer_2)
  ...
  describe '#compose_transaction' do
    ...
    it 'composes multi-instruction transaction with account deduplication' do
      composer.add_instruction(transfer_composer_1)
      composer.add_instruction(transfer_composer_2)
