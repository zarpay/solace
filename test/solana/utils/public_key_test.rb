...
describe Solana::Utils::PublicKey do
  before do
    # Example 32-byte key (all 1s)
    @bytes = Array.new(32, 1)
    @base58 = Solana::Utils::Codecs.bytes_to_base58(@bytes)
    @public_key = Solana::Utils::PublicKey.new(@bytes)
  end
  ...
  describe '#initialize' do
    ...
    it 'raises on invalid length' do
      assert_raises(ArgumentError) { Solana::Utils::PublicKey.new([1,2,3]) }
    end
  end
  ...
  describe '#==' do
    it 'returns true for equal keys' do
      pk2 = Solana::Utils::PublicKey.new(@bytes.dup)
      refute_nil pk2
      assert_equal @public_key, pk2
    end

    it 'returns false for different keys' do
      pk3 = Solana::Utils::PublicKey.new(Array.new(32, 2))
